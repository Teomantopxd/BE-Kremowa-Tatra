...
class ProductsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        with open(self.output_filename, 'a', newline='', encoding='utf-8') as file:
            fieldnames = ['name', 'price', 'image_url', 'quantity', 'description', 'category', 'tax', 'power']
            ...
            for product in response.css('.item.AjaxBasket'):
                power = re.search(pattern, product.css('.productName a span::text').get())
                item = {
                    ...
                    'tax': self.tax,
                    'power': "Moc:" + power.group(0) + '%' if power else "Moc: 0%"
                }
    ...
    def parse_product(self, response):
        ...
        raw_quantity = int(quantity.group()) if quantity else 0
